...
const Modal = ({ brand, title, desc, onClose }) => {
  const { i18n } = useTranslation();

  if (!brand) return null;

  // إغلاق النافذة لو ضغط المستخدم بالخلفية المظلمة (برا المربع)
  ...
  const whatsappMessage = i18n.language === 'ar' 
    ? `مرحباً مجموعة يزن، أود الاستفسار عن (${title})`
    : `Hello Yazan Group, I would like to inquire about (${title})`;

  // تحويل الرسالة لصيغة يقبلها رابط الإنترنت
    ...
    <div className="modal-overlay" onClick={handleOverlayClick}>
      <div className="modal-card">
        ...
        <div className="modal-body">
          <h3 className="text-gradient-gold">{title}</h3>
          <p>{desc}</p>
          ...
          <a href={whatsappLink} target="_blank" rel="noreferrer" className="whatsapp-btn">
            ...
            {i18n.language === 'ar' ? 'تواصل عبر الواتساب' : 'Contact on WhatsApp'}
          </a>
